...
class AnnotationToolSurface:
    ...
    def query_telemetry(self, query_id: str, params_json: str) -> str:
        from training.shared.annotation_agent_tools import run_pipeline_query
        try:
    ...
    def compute_expert_phases(self, start: int, end: int) -> str:
        from training.shared.annotation_agent_tools import compute_expert_phases
    ...
    def locate_circuit_section(self, circuit_id: str, start: int, end: int) -> str:
        from training.shared.annotation_agent_tools import locate_circuit_section
        s, e = self._clamp_to_window(start, end)
        data = locate_circuit_section(self.df, circuit_id, s, e)
        return json.dumps({"range": [s, e], "data": data}, default=str)

    def find_nearest_opponent(self, start: int, end: int) -> str:
        from training.shared.annotation_agent_tools import find_nearest_opponent
        s, e = self._clamp_to_window(start, end)
    ...
    def classify_opponent_interaction(self, start: int, end: int) -> str:
        from training.shared.annotation_agent_tools import classify_opponent_interaction
        s, e = self._clamp_to_window(start, end)
    ...
    def query_opponent_trajectory(self, start: int, end: int, slot: int, n_samples: int) -> str:
        from training.shared.annotation_agent_tools import query_opponent_trajectory
        s, e = self._clamp_to_window(start, end)
