...
from training.storage.cache import model_cache_service
...
class Full_dataset_TelemetryMLService:
    ...
    def __init__(self, models_directory: Optional[str] = None, logger: Optional[logging.Logger] = None, pipeline_config: Optional[TrainingPipelineConfig] = None):
        ...
        if models_directory:
            ...
            # __file__ = training/pipelines/training/full_dataset.py → parents[3]
        ...
        try:
            self._top_lap_reference_feature_names = (
                self.telemetry_features.get_features_for_top_lap_reference()
            )
        except Exception:
            self._top_lap_reference_feature_names = []
        ...
        # Telemetry store backed by Lance; see training/storage/__init__.py.
...
if __name__ == "__main__":
    # Example usage
    print("TelemetryMLService with top-lap references initialized. Ready for training!")
