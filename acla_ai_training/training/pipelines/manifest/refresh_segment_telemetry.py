...
input source so the new columns land on the saved segment dicts.
    ...
    python -m training.pipelines.manifest.refresh_segment_telemetry <pipeline_id> <node_id>
"""
...
from training.pipelines.manifest.registry import load as load_pipeline
from training.pipelines.manifest.segment_refresh import refresh_node_segments
from training.storage.lance import get_shared_lance_store
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        raise SystemExit(
            "Usage: python -m training.pipelines.manifest.refresh_segment_telemetry "
            "<pipeline_id> <node_id>"
        )
