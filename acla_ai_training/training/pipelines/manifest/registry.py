...
Manifests are JSON files in ``acla_ai_training/storage/pipelines/``. One file per
...
from training.pipelines.manifest.models import Pipeline


_STORAGE_ROOT = Path(os.environ.get(
    "PIPELINE_STORAGE_DIR",
    Path(__file__).resolve().parents[3] / "storage" / "pipelines",
))
