...
exposes the same public surface as ``training.storage.zarr.ZarrTelemetryStore``
...
from :mod:`training.storage.lance.strategies`. Telemetry-shaped keys
...
from training.storage.cache_metadata import CacheMetadata, ChunkPayload
from training.storage.lance.strategies import (
    SchemaStrategy,
    BlobStrategy,
...
def _format_chunk_name(index: Union[int, str]) -> str:
    """Mirror :func:`training.storage.zarr._format_chunk_name`.
    ...
    def __init__(
        ...
        *,
        ...
        if store_directory:
            ...
            # Keep datasets outside the Python package in the training workspace.
            self.store_dir = Path(os.environ.get(
                "TELEMETRY_STORE_DIR",
                Path(__file__).resolve().parents[3] / "storage" / "telemetry_lance_store",
            ))
