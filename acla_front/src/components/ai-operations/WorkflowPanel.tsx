...
import ProcedurePlan, { useProcedurePlanWorkflow, type ProcedurePlanHandle, type ProcedurePlanInput } from './ProcedurePlan';
import RepeatablePlan, { useRepeatablePlanWorkflow, type RepeatablePlanHandle, type RepeatablePlanInput } from './RepeatablePlan';
...
import type { ToolDispatcher } from './tool';
...
export interface WorkflowPanelHandle extends NamedOperationComponentHandle {
    createProcedurePlan(input: ProcedurePlanInput, dispatch: ToolDispatcher): ReturnType<ProcedurePlanHandle['createProcedurePlan']>;
    createRepeatablePlan(input: RepeatablePlanInput, dispatch: ToolDispatcher): ReturnType<RepeatablePlanHandle['createRepeatablePlan']>;
...
const WorkflowPanel = forwardRef<WorkflowPanelHandle, WorkflowPanelProps>(({
    ...
    const mountWorkflow = useCallback<MountWorkflow>((workflow) => {
        ...
        if (previous && !previous.retainOnHide) previous.dispose();
    ...
    const handle = useMemo<WorkflowPanelHandle>(() => ({
        ...
        reset,
    }), [procedure.createProcedurePlan, procedure.handleUserText, procedure.handleToolStatus,
        repeatable.createRepeatablePlan, liveRange.initializeLiveRangeTodoList, reset]);
