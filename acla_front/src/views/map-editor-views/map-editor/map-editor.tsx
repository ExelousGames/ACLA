...
import { ZoomInIcon, ZoomOutIcon, Pencil1Icon, TrashIcon, DotFilledIcon, TriangleRightIcon, CursorArrowIcon, PlayIcon, StopIcon } from '@radix-ui/react-icons';
import { MapEditorContext } from '../map-editor-view';
import SettingsMenu from './components/SettingsMenu';
import AutoTurningPointGenerator from './components/AutoTurningPointGenerator';
import { RacingTurningPoint, CurbTurningPoint, BezierPoints, RacingLinePoint } from './types';
...
const MapEditor = () => {
    ...
    // Track which point is currently selected for editing/inspecting
    ...
    const handleAutoGeneratedPoints = useCallback((generatedPoints: RacingTurningPoint[]) => {
        const normalizedPoints: RacingTurningPoint[] = generatedPoints.map((point, index) => ({
            ...point,
            index
        }));

        setTurningPoints(normalizedPoints);
        setSelectedPointIndex(normalizedPoints.length ? normalizedPoints[0].index : null);
    }, []);
    ...
    const handleStageMouseDown = (e: any) => {
        if (!isTargetInteractive(e.target)) {
            // We clicked on draggable area (empty space, background image, or racing line), allow stage dragging and clear selection
            setIsDraggingPoint(false);
    ...
    function createInitialShapes() {
        ...
        apiService.post('/racingmap/map/infolists', { name: mapEditorContext.mapSelected }).then((result) => {
            ...
            if (!data.points || (data.points as any).length < 4) {
                ...
                setSelectedPointIndex(circlePoints.length ? circlePoints[0].index : null);
            } else {
                const normalized: RacingTurningPoint[] = data.points.map((point) => {
                    return {
                        type: point.type,
                        index: point.index,
                        position: [point.position[0], point.position[1]] as Point,
                ...
                setTurningPoints(normalized);
                setSelectedPointIndex(normalized.length ? normalized[0].index : null);
            }
    ...
    const handleDragStart = (e: any, id: any) => {
        ...
        setIsDraggingPoint(true);
        setSelectedPointIndex(id);
    };

    const handlePointSelect = useCallback((e: any, id: number) => {
        e.cancelBubble = true;

        const stage = e.target.getStage?.();
        const nodeIsDragging = typeof e.target.isDragging === 'function' && e.target.isDragging();
        const stageIsDragging = stage && typeof stage.isDragging === 'function' && stage.isDragging();

        if (nodeIsDragging || stageIsDragging) {
            return;
        }

        setSelectedPointIndex(prev => (prev === id ? null : id));
    }, []);
    ...
    */
    function AddPointInDirection() {
        ...
        setTurningPoints(prev => [...prev, newP]);
        setSelectedPointIndex(newP.index);
    }
    ...
    function deleteTurningPoint(index: number) {
        ...
        setSelectedPointIndex(prev => (prev === index ? null : prev));
    }
    ...
    return (
        ...
        <div ref={containerRef} style={{ width: '100%', height: '90%', position: 'relative' }}>
            ...
            {mapEditorContext.mapSelected && (
                <AutoTurningPointGenerator
                    mapImage={uploadedMapImage ?? mapImage ?? null}
                    stageSize={stageSize}
                    onGenerate={handleAutoGeneratedPoints}
                    ...
                    <div>
                        ...
                        >
                            <Layer>
                                ...
                                {
                                    turningPoints.map((turningPoint: RacingTurningPoint) => {
                                        const pointStyle = getPointStyling(turningPoint.type);
                                        const IconComponent = pointStyle.icon;
                                        const isSelected = selectedPointIndex === turningPoint.index;
                                        ...
                                        return (
                                            <Group
                                                ...
                                                onClick={(e) => handlePointSelect(e, turningPoint.index)}
                                                onTap={(e) => handlePointSelect(e, turningPoint.index)}
                                            >
                                                {/* Selection halo */}
                                                {isSelected && (
                                                    <Circle
