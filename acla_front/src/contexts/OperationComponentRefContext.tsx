import React, {
    MutableRefObject,
    createContext,
    useContext,
    useLayoutEffect,
    useMemo,
    useRef,
    useState,
    useSyncExternalStore,
} from 'react';
import AiOverlayManager from 'views/floating-chat/AiOverlayManager';
import {
    OperationComponentRefError,
    ComponentMountTimeoutError,
    ComponentRefUnavailableError,
    DuplicateComponentNameError,
} from './OperationComponentError';

export {
    OperationComponentRefError,
    ComponentMountTimeoutError,
    ComponentRefUnavailableError,
    DuplicateComponentNameError,
} from './OperationComponentError';

export const OPERATION_COMPONENT_MOUNT_TIMEOUT_MS = 5000;

export const OPERATION_COMPONENT_NAMES = Object.freeze({
    DASHBOARD_ASSISTANT: 'dashboard-assistant',
    WORKFLOW_PANEL: 'workflow-panel',
    LIVE_SESSION: 'live-session',
    SESSION_ANALYSIS: 'session-analysis',
    USER_SUMMARY: 'user-summary',
    LIVE_VISUALIZATION_MANAGER: 'live-visualization-manager',
    RECORDED_VISUALIZATION_MANAGER: 'recorded-visualization-manager',
    LIVE_RANGE_TODO_LIST: 'live-range-todo-list',
    REPEATABLE_PLAN: 'repeatable-plan',
    PROCEDURE_PLAN: 'procedure-plan',
    BASELINE_COLLECTION: 'baseline-collection',
});

export interface NamedOperationComponentHandle {
    getComponentName(): string;
}

export interface ObservableOperationComponentHandle<TSnapshot> extends NamedOperationComponentHandle {
    getAssistantSnapshot(): TSnapshot;
    subscribeAssistantSnapshot(listener: () => void): () => void;
}

export type OperationComponentRef<THandle extends NamedOperationComponentHandle = NamedOperationComponentHandle> =
    MutableRefObject<THandle | null>;

type ComponentWaiter = {
    resolve: (ref: OperationComponentRef) => void;
    reject: (error: OperationComponentRefError) => void;
    timeoutId: ReturnType<typeof setTimeout>;
};

export interface OperationComponentRefDirectory {
    findComponentRef<THandle extends NamedOperationComponentHandle>(name: string): OperationComponentRef<THandle> | null;
    registerComponentRef<THandle extends NamedOperationComponentHandle>(ref: OperationComponentRef<THandle>): void;
    unregisterComponentRef(ref: OperationComponentRef): boolean;
    awaitComponentRef<THandle extends NamedOperationComponentHandle>(
        name: string,
        timeoutMs?: number,
    ): Promise<OperationComponentRef<THandle>>;
    getComponentNames(): string[];
    getComponentRefs(): OperationComponentRef[];
}

export const createOperationComponentRefDirectory = (
    onChange: () => void = () => undefined,
): OperationComponentRefDirectory => {
    const entries = new Map<string, OperationComponentRef>();
    const namesByRef = new Map<OperationComponentRef, string>();
    const waiters = new Map<string, Set<ComponentWaiter>>();

    const findComponentRef = <THandle extends NamedOperationComponentHandle>(name: string) => (
        (entries.get(name) as OperationComponentRef<THandle> | undefined) ?? null
    );

    const registerComponentRef = <THandle extends NamedOperationComponentHandle>(
        ref: OperationComponentRef<THandle>,
    ): void => {
        const handle = ref.current;
        if (!handle) {
            throw new ComponentRefUnavailableError(
                '(unmounted)',
                'A component reference must be mounted before it can be registered.',
            );
        }
        const componentName = handle.getComponentName();
        if (!componentName.trim()) {
            throw new ComponentRefUnavailableError(
                componentName,
                'A component must return a non-empty runtime name.',
            );
        }

        const directoryRef = ref as OperationComponentRef;
        const registeredName = namesByRef.get(directoryRef);
        if (registeredName === componentName && entries.get(componentName) === ref) return;
        if (registeredName) {
            entries.delete(registeredName);
            namesByRef.delete(directoryRef);
        }

        const existing = entries.get(componentName);
        if (existing && existing !== ref) {
            throw new DuplicateComponentNameError(
                componentName,
                `A mounted component already owns '${componentName}'.`,
            );
        }

        entries.set(componentName, directoryRef);
        namesByRef.set(directoryRef, componentName);
        const pending = waiters.get(componentName);
        if (pending) {
            pending.forEach((waiter) => {
                clearTimeout(waiter.timeoutId);
                waiter.resolve(directoryRef);
            });
            waiters.delete(componentName);
        }
        onChange();
    };

    const unregisterComponentRef = (ref: OperationComponentRef): boolean => {
        const componentName = namesByRef.get(ref);
        if (!componentName || entries.get(componentName) !== ref) return false;
        entries.delete(componentName);
        namesByRef.delete(ref);
        onChange();
        return true;
    };

    const awaitComponentRef = <THandle extends NamedOperationComponentHandle>(
        name: string,
        timeoutMs = OPERATION_COMPONENT_MOUNT_TIMEOUT_MS,
    ): Promise<OperationComponentRef<THandle>> => {
        const existing = findComponentRef<THandle>(name);
        if (existing?.current) return Promise.resolve(existing);

        return new Promise((resolve, reject) => {
            const waiter: ComponentWaiter = {
                resolve: (ref) => resolve(ref as OperationComponentRef<THandle>),
                reject,
                timeoutId: setTimeout(() => {
                    const current = waiters.get(name);
                    current?.delete(waiter);
                    if (current?.size === 0) waiters.delete(name);
                    reject(new ComponentMountTimeoutError(
                        name,
                        `Component '${name}' did not mount within ${timeoutMs}ms.`,
                    ));
                }, timeoutMs),
            };
            const current = waiters.get(name) ?? new Set<ComponentWaiter>();
            current.add(waiter);
            waiters.set(name, current);
        });
    };

    return {
        findComponentRef,
        registerComponentRef,
        unregisterComponentRef,
        awaitComponentRef,
        getComponentNames: () => Array.from(entries.keys()).sort(),
        getComponentRefs: () => Array.from(entries.values()),
    };
};

interface OperationComponentRefContextValue {
    directory: OperationComponentRefDirectory | null;
    revision: number;
}

const OperationComponentRefContext = createContext<OperationComponentRefContextValue>({
    directory: null,
    revision: 0,
});

export const OperationComponentRefProvider = ({ children }: { children: React.ReactNode }) => {
    const [revision, setRevision] = useState(0);
    const directoryRef = useRef<OperationComponentRefDirectory | null>(null);
    if (directoryRef.current === null) {
        directoryRef.current = createOperationComponentRefDirectory(() => {
            setRevision((current) => current + 1);
        });
    }
    const value = useMemo(() => ({ directory: directoryRef.current, revision }), [revision]);

    return (
        <OperationComponentRefContext.Provider value={value}>
            <AiOverlayManager directory={directoryRef.current} directoryRevision={revision} />
            {children}
        </OperationComponentRefContext.Provider>
    );
};

export const useOperationComponentRefs = () => {
    const value = useContext(OperationComponentRefContext);
    const directory = value.directory;
    if (!directory) throw new Error('OperationComponentRefProvider is required.');
    return { directory, revision: value.revision };
};

export const useOptionalOperationComponentSnapshot = <TSnapshot,>(
    name: string | null | undefined,
): TSnapshot | null => {
    const { directory } = useContext(OperationComponentRefContext);
    const handle = name && directory
        ? directory.findComponentRef<ObservableOperationComponentHandle<TSnapshot>>(name)?.current ?? null
        : null;
    const subscribe = React.useCallback((listener: () => void) => (
        handle?.subscribeAssistantSnapshot(listener) ?? (() => undefined)
    ), [handle]);
    const getSnapshot = React.useCallback(
        () => handle?.getAssistantSnapshot() ?? null,
        [handle],
    );

    return useSyncExternalStore(subscribe, getSnapshot, getSnapshot);
};

export const useOptionalOperationComponentRefDirectory = (): OperationComponentRefDirectory | null => (
    useContext(OperationComponentRefContext).directory
);

export const useOperationComponentRefDirectory = (): OperationComponentRefDirectory => (
    useOperationComponentRefs().directory
);

export const useRegisterOperationComponentRef = <THandle extends NamedOperationComponentHandle>(
    ref: OperationComponentRef<THandle>,
) => {
    const { directory } = useContext(OperationComponentRefContext);

    useLayoutEffect(() => {
        if (!directory) return undefined;
        directory.registerComponentRef(ref);
        return () => {
            directory.unregisterComponentRef(ref);
        };
    }, [directory, ref]);
};

export const resolveNamedComponentHandle = <THandle extends NamedOperationComponentHandle>(
    directory: OperationComponentRefDirectory,
    name: string,
): THandle => {
    const handle = directory.findComponentRef<THandle>(name)?.current;
    if (!handle) {
        throw new ComponentRefUnavailableError(
            name,
            `Component '${name}' is not mounted in the active dashboard.`,
        );
    }
    return handle;
};

export const awaitNamedComponentHandle = async <THandle extends NamedOperationComponentHandle>(
    directory: OperationComponentRefDirectory,
    name: string,
    timeoutMs = OPERATION_COMPONENT_MOUNT_TIMEOUT_MS,
): Promise<THandle> => {
    await directory.awaitComponentRef<THandle>(name, timeoutMs);
    const handle = directory.findComponentRef<THandle>(name)?.current;
    if (!handle) {
        throw new ComponentRefUnavailableError(
            name,
            `Component '${name}' unmounted before it could receive the command.`,
        );
    }
    return handle;
};

export type AiChatAssistantMode = 'front_desk' | 'live' | 'recorded' | 'user_summary';
