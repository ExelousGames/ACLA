...
def render_batch_auto_annotation(df, selected_annotation_key):
    ...
    """
    with st.expander("Batch Auto-Annotation (Gemini/Service)"):
        ...
        with col_b1:
            auto_apply = st.checkbox("Auto-apply Suggested Labels", value=False, key="batch_auto_apply_chk")
                ...
                if st.session_state.get("start_batch_requested", False) and not is_running:
                    ...
                    if not api_key:
                        ...
                        def persistence_callback(idx, new_label_ids, response_text):
                            # Update Global State safely (relies on thread-safety of python lists/dicts, generally ok for simple appends)
                            # Accessing st.session_state from thread context
                            if "current_annotations" in st.session_state:
                                annotations = st.session_state.current_annotations
                                if 0 <= idx < len(annotations):
                                    ann = annotations[idx]
                                    
                                    # Update Labels (if auto-apply enabled)
                                    # Dynamically check the checkbox state in case user changed it during run
                                    should_apply = st.session_state.get("batch_auto_apply_chk", auto_apply)
                                    
                                    if should_apply:
                                        current_set = set(ann.labels)
                                        added = [nl for nl in new_label_ids if nl not in current_set]
                                        if added:
                                            ann.labels.extend(added)
                                    
                                    # Update Notes (Overwrites existing notes with full prompt result)
                                    if response_text:
                                        ts = time.strftime("%Y-%m-%d %H:%M:%S")
                                        # Overwrite existing notes completely
                                        ann.notes = f"--- AI Analysis ({ts}) ---\n{response_text}"
                                        
                                    # We don't save to disk here to avoid race conditions or lag
                                    # The user or "Force Save" will persist.
...
def render_detailed_labeling(selected_annotation_key, selected_session_key, available_sessions):
    ...
    # --- Batch Auto-Annotation (Placed Above Manage Section) ---
