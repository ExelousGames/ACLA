...
logger = logging.getLogger("open_pipeline_management")
...
def main() -> None:
    log_message("\n" + "=" * 50)
    log_message(" Pipeline Management")
    log_message("=" * 50)
    log_message("Opening Streamlit pipeline management for data preparation, annotation, and training...")
    ...
    log_message("\n" + "=" * 50)
    log_message(" Pipeline Management Closed")
    log_message("=" * 50)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Open ACC pipeline management")
    parser.add_argument("--log-file", type=str, help="Optional path to a log file. Defaults to logs/pipeline_management_<timestamp>.log")
    ...
    if args.log_file:
        ...
        log_path = default_dir / f"pipeline_management_{timestamp}.log"

    setup_logging(log_path)
    logger.info("Pipeline management logs will be written to %s", log_path)
