...
async def test_live_baseline_analysis_returns_circuit_sections_without_classifier_children(monkeypatch):
    ...
    assert [segment["parent_segment_id"] for segment in result["segments"]] == [
        "brands_hatch2",
        "brands_hatch3",
    ]
...
def test_annotate_segments_uses_parent_segment_range_for_time_gap():
    segments = [
        {
            "parent_segment_id": "brands_hatch1",
            "start_index": 0,
            "end_index": 5,
            "child_segments": [
                {
