...
Exposes one public entry point:
    ...
    from app.agents import run_agent, AgentRequest, AgentResponse
...
def run_agent(request: AgentRequest) -> AgentResponse:
    ...
    if request.backend == "local":
        from app.agents.runners.local import run_local
        return run_local(request)
    if request.backend == "claude":
        from app.agents.runners.claude import run_claude
