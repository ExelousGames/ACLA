...
class ClaudeLapAnnotationRunner:
    ...
    def _build_system_prompt(self) -> str:
        ...
        thinking_clause = (
            "\nThink step-by-step before each tool call: state which "
            "shrink/extend rule (if any) you are checking, pick the most "
            "direct tool, read the result before deciding the next step.\n"
        ...
        return (
            ...
            "rough iloc boundary; your job is to (a) decide whether the "
            "boundary should be shrunk / extended based on the player "
            "telemetry, and (b) pick the parent labels for the section.\n"
            "\n"
            "### Lap context\n"
            ...
            "Pick the circuit, the section (override only when shrink/extend "
            "pushed the boundary into a different catalog range), one ST1–ST6, "
            "and an optional main label.\n"
            + "\n".join(eligible_lines)
            + "\n\n"
            ...
            + "\n\n"
            "### How to work\n"
            "1. **Read the section description and shrink/extend rules in the skill block above.** "
            "Decide which rules might fire for this section.\n"
            "2. Render ONE trajectory graph (e.g. `trajectory_detailed` or `trajectory_offset`) over the rough boundary to confirm the player drove through the section.\n"
            "3. If a shrink/extend rule fires, use `query_telemetry` to find the exact iloc that triggers it (e.g. `find_first_match` on `Physics_brake` to locate brake onset), then call `revise_segment_range(new_start, new_end)`. Validate against the lap range and the 3-iloc minimum.\n"
            "4. Pick the segment_type label (ST1–ST6) using the skill hints. Call `compute_expert_phases` when in doubt — empty ⇒ ST2; one arc ⇒ ST1 / ST3 / ST4; multi-arc no gap ⇒ ST6.\n"
            "5. Call `submit_lap_proposal(label_ids_json, reasoning)` with the chosen IDs and a 1-3 sentence reasoning citing ilocs.\n"
            "6. After `submit_lap_proposal` returns `ok: true`, stop.\n"
            "\n"
            "### Hard rules\n"
