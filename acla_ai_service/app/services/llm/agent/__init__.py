...
Exposes one public entry point:
    ...
    from app.services.llm.agent import run_agent, AgentRequest, AgentResponse
...
def run_agent(request: AgentRequest) -> AgentResponse:
    ...
    if request.backend == "local":
        from app.services.llm.agent.runners.local import run_local
        return run_local(request)
    if request.backend == "claude":
        from app.services.llm.agent.runners.claude import run_claude
