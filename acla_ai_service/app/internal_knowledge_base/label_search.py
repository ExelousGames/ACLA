...
_CACHE_DIR = _PACKAGE_ROOT.parents[1] / ".cache" / "labels_llama"
...
def _build_or_load() -> None:
    ...
    from app.internal_knowledge_base._embedder import get_llama_embedding
    ...
    _retriever = QueryFusionRetriever(
        ...
        num_queries=1,
        use_async=False,
        # num_queries=1 means no query generation, but the retriever still
        # resolves Settings.llm in __init__ (defaults to OpenAI). MockLLM
        # avoids the llama-index-llms-openai dependency; it's never called.
        llm=MockLLM(),
    )
