...
def _build_system_prompt(request: AgentRequest) -> str:
    ...
    return (
        ...
        "pass-event flags. Inline close-following cars may matter even "
        "when they never become side-by-side. Use as supporting context "
        "for O / OD / MSR.\n"
        ...
        "`broken_defense`, `close_following`, etc.), recommended label, "
        "numeric confidence, confidence_level, primary slot, raw outcome gates, "
        ...
        "committing. Treat a close car tucked directly behind or ahead as "
        "interaction evidence even if there is no side-by-side overlap, "
        "but do not treat `close_following` alone as O / OD / MSR. Inline "
        "rear pressure is following, not OD; held defense needs an actual "
        "lateral/alongside threat.\n"
...
def _build_tool_set(surface: _ToolSurface):
    ...
    @tool(
        ...
        "minimum lateral offset, side-by-side iloc count, close-following "
        "evidence, and `passed_by_player` / `got_passed_by_opponent` flags. Use after "
    ...
    @tool(
        ...
        "(`pass_completed`, `held_defense`, `failed_attack`, "
        "`broken_defense`, `close_following`, etc.), `recommended_label` "
        "(O / OD / MSR / null), numeric confidence, confidence_level, "
        "primary opponent slot, "
        "raw outcome gates, confidence-aware label_gates, and per-slot "
        "evidence. Inline close-following cars can be relevant without "
        "side-by-side overlap, but `close_following` alone is target-car "
        "context and does not gate O / OD / MSR. Inline rear pressure is "
        "following, not OD; held defense needs an actual lateral/alongside "
        "threat. Use as the mathematical source of truth before choosing "
