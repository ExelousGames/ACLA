...
(initiate throttle too early) or too aggressive a ramp (throttle applied too quickly).
...
- Throttle trace shows early or steep build (initiate throttle too early / throttle applied too quickly).
...
## Remedies
- Delay throttle-on until steering is unwinding (initiate throttle too early).
- Gentler throttle ramp (throttle applied too quickly).
- If repeatable across corners, the rear may need more grip —
  setup question for the engineer.
- A wider apex (apex too wide) often cures power oversteer on RWD cars by
