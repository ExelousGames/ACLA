...
## Telemetry signature
- `find_nearest_opponent` returns a close primary candidate directly
  behind (`entry_signed_long_gap_m < 0`, small `min_distance_m`);
  `query_opponent_trajectory` shows the signed longitudinal gap
  crossing zero across the straight.
...
- `time_difference_to_expert` grows during and just after the lift.
...
- If the lift didn't break the tow, don't compound it with a tight
  defensive line into the corner that becomes MSR7. Set up the
