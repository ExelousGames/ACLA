...
becomes MSR3 — a failed late-brake).

## Telemetry signature
- `find_nearest_opponent` returns a primary candidate directly ahead
  with small `min_lateral_offset_m` (in-line, not side-by-side) and
  `entry_signed_long_gap_m` positive and small (typically < ~30 m).
- `passed_by_player: false` AND `got_passed_by_opponent: false`.
- `query_opponent_trajectory` shows the signed longitudinal gap
  shrinking monotonically across the straight but **not crossing
  zero**.
- Player speed delta shows **player faster than expert** with
  throttle **at or below expert** — the speed gain is draft, not
  input.
- Sometimes followed by an MSR3 signature into the next brake zone
  (panic late-brake when the straight runs out).
...
- If the straight runs out and the pass isn't there, brake on the
  ...
  desperate late-brake that becomes MSR3.
