...
the same amount of speed but starting at a higher peak (MS22), or
the brake gets compressed into less distance with a fast ramp
(MS14). Either way the brake phase ends up uglier.

## Telemetry signature
- Throttle still positive past expert's lift-off marker.
- May pair with MS34 (brake+throttle overlap) if the brake came
  on before the throttle finished releasing.
- Top speed before brake zone above expert.
- Brake phase compressed (MS22, MS14 downstream).
