...
const MODEL_COMMAND_DEFINITIONS = [
    {
        ...
        description: [
            'Append validated steps to the remaining procedure sequence. If missing, start independent execution with a goal derived from the first step title and acknowledge immediately. Use the native tool call channel with workflow as its single outer key. No legacy compatibility. Create replaces only its own workflow type and waits for execution to finish. Creation cannot replace the executing workflow or an active ancestor; independent same-type replacement is allowed. Append acknowledges insertion immediately and may target itself or an independent workflow, but never an ancestor or descendant. Showing a workflow preserves other types and does not automatically restore a parent.',
...
\`\`\``,
    ...
    },
    {
        ...
        description: [
            'Append validated steps to the current and future passes. If a check or retry delay is underway, execute appended steps and check again before completing. Reject when no repeatable plan exists; create must supply the stop condition. Use the native tool call channel with workflow as its single outer key. No legacy compatibility. Create replaces only its own workflow type and waits for execution to finish. Creation cannot replace the executing workflow or an active ancestor; independent same-type replacement is allowed. Append acknowledges insertion immediately and may target itself or an independent workflow, but never an ancestor or descendant. Showing a workflow preserves other types and does not automatically restore a parent.',
            `Example native arguments:
\`\`\`json
{
  "workflow": {
...
\`\`\``,
    ...
    },
    {
        ...
        description: [
            'Create or replace the session live-range queue. Keep telemetry scheduling and failure handling. The operation stays pending until all events, including appended events, finish; execution errors are reported when the run finishes. Use the native tool call channel with workflow as its single outer key. No legacy compatibility. Create replaces only its own workflow type and waits for execution to finish. Creation cannot replace the executing workflow or an active ancestor; independent same-type replacement is allowed. Append acknowledges insertion immediately and may target itself or an independent workflow, but never an ancestor or descendant. Showing a workflow preserves other types and does not automatically restore a parent.',
            `Example native arguments:
\`\`\`json
{
  "workflow": {
...
\`\`\``,
    ...
    },
    {
        ...
        description: [
            'Evaluate a JSONata expression against the final displayed Analysis Results without rerunning analysis. scope is "all" (the default) or a 1-based page number in retained-page array order. Each page keeps its own View and applied filter. First select the scoped pages and use each page\'s final filtered and ordered elements, then evaluate the query. Pending page filters finish before querying. Querying does not change the active page or any filter. For example, scope "all" combines page 1 with its filter and page 2 with its own unfiltered All results view. Invalid or missing page numbers are rejected with INVALID_QUERY_SCOPE.',
            'The expression receives exactly one root structure: { "analyses": [{ "id": "...", "createdAt": 0, "sourceIndex": 0, "baseline": { "lap_id": 1, "lapTimeMs": 0, "track": "...", "car": "..." }, "elements": [{ "id": "...", "labels": [{ "label_name": "...", "start_index": 0, "end_index": 1 }], "title": "...", "section": "...", "normalizedPositionRange": { "start": 0, "end": 1 }, "timeGap": { "startMs": 0, "endMs": 0, "deltaMs": 0 }, "comparison": { "samples": [{ "driverTimeMs": 0, "expertTimeMs": 0, "driverTrackPosition": 0, "expertTrackPosition": 0, "driverTrajectory": { "x": 0, "y": 0, "z": 0 }, "expertTrajectory": { "x": 0, "y": 0, "z": 0 }, "driverGas": 0, "expertGas": 0, "driverBrake": 0, "expertBrake": 0, "driverGear": 1, "expertGear": 1 }] }, "metadata": { "source": "ai_classifier", "start_index": 0, "end_index": 1 } }] }] }. analyses contains the scoped pages in displayed order, including pages whose filters match no elements. For a non-paginated recorded result it contains one analysis with null createdAt and baseline.',
            'In this structure, createdAt and baseline may be null, and baseline.lapTimeMs may be null. sourceIndex is the original zero-based displayed array index even when scoped to one page, and baseline.lap_id is a non-negative telemetry lap identifier. Only id and labels are required on each element; title, section, normalizedPositionRange, timeGap, comparison, and metadata are optional. timeGap fields are optional. Comparison samples require driverTimeMs, expertTimeMs, driverTrackPosition, and expertTrackPosition; the other sample fields are optional, and trajectory points require only x. timeGap, comparison, and metadata are JSON objects that may contain additional fields.',
            ...
            'Each query independently enforces a maximum of 8,192 bytes of compact UTF-8 JSON for the entire { "status": "ready", "data": ... } payload, excluding the transport envelope, and a maximum of 50 items in every returned array, including nested arrays. Oversized results are rejected completely with QUERY_RESULT_LIMIT_EXCEEDED; no partial data is returned. Filter the results, select fewer fields, or aggregate to fit these limits. There are no caller-controlled output-limit overrides or output pagination parameters.',
            'JSONata can calculate over all scoped, filtered analysis data before these output limits are applied. Small complete datasets and repeated bounded queries are allowed. Normalized error details are limited to 1,024 serialized bytes; oversized diagnostics are replaced with a fixed error without the original message or cause.',
            'Examples: $count(analyses) counts analyses in scope, including empty pages; $count(analyses.elements) counts displayed segments in scope; analyses.elements[labels[label_name = "Lockup"]].{ "id": id, "section": section }.',
        ].join(' '),
        properties: {
            query: {
                type: 'string',
            ...
            scope: {
                ...
                default: 'all',
                description: 'Pages to query after their own View and applied filter: "all" (default) or a 1-based displayed page number. A recorded result has only page 1.',
            },
        },
        required: ['query'],
    },
...
const expandWorkflowSchemas = (commands: readonly ModelCommandDefinition[]): ModelCommandSchema[] => {
    ...
    return commands.map((command) => {
        ...
        if (
            ...
            && command.name !== 'create_live_range_todo_list'
            && command.name !== 'append_procedure_plan'
